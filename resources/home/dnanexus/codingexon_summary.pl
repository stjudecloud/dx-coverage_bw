#!/usr/bin/env perl
###############################################################################
## Name:    codingexon_summary.pl                                             #
## Author:  Gang Wu                                                           #
## Date:    Aug 1st, 2010                                                     #
## Purpose: get the mean coverage for all coding exons on all chromosomes     #
## This was based on exon_summary.pl, this version deals with *.CodingExon.txt#
## Usage:   perl codingexon_summary.pl F SJMB001_D \                          #
##  /nfs_exports/genomes/1/PCGP/BrainTumors/SJMB/NextGen/WholeGenome/Coverage/presum
##  /nfs_exports/apps/gnu-apps/NextGen/nextgensupport/hg18
###############################################################################
...
my ($gender,$prefix,$cov_dir,$annot_dir) = @ARGV;
...
my @files = glob ("$cov_dir/$prefix*.CodingExon.txt");
foreach my $f (@files) {
...
open(OUT,">$prefix.codingexon.coverage.means.txt") or die "cant open file to write\n";
...
open(OUT,">$prefix.codingexon.coverage.counts.txt") or die "cant open file to write\n";
print OUT "Coverage";
my @chrs = (1..22,"X","Y");
...
open(OUT,">$prefix.codingexon.coverage.means.nr.sorted.txt") or die "cant open file to write\n";
foreach my $gae (sort {$nr_means{$a} <=> $nr_means{$b}}  keys %nr_means) {
	print OUT "$gae\t$nr_means{$gae}\n";
}
close OUT;

open(OUT,">$prefix.codingexon.coverage.means.with.GC.txt") or die "cant open file to write\n";
